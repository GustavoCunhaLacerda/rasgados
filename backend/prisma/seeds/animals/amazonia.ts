...
export async function amazonia() {
  ...
  const botoCorDeRosaIcon = buildPrismaImage(
    animals["boto_cor_de_rosa"]["icon"],
  );
  const botoCorDeRosaImages = animals["boto_cor_de_rosa"]["images"].map((
    image: string,
  ...
  const harpiaImages = animals["harpia"]["images"].map((image: string) =>
    buildPrismaImage(image)
  );
  ...
  const macacoAranhaDaCaraPretaIcon = buildPrismaImage(
    animals["macaco_aranha_da_cara_preta"]["icon"],
  );
  const macacoAranhaDaCaraPretaImages =
    animals["macaco_aranha_da_cara_preta"]["images"].map((image: string) =>
      buildPrismaImage(image)
    );
  ...
  const oncaPintadaImages = animals["onca_pintada"]["images"].map((
    image: string,
  ) => buildPrismaImage(image));
  ...
  const peixeBoiDaAmazoniaIcon = buildPrismaImage(
    animals["peixe_boi_da_amazonia"]["icon"],
  );
  const peixeBoiDaAmazoniaImages = animals["peixe_boi_da_amazonia"]["images"]
